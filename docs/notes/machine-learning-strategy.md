...
- For a supervised learning system to do well, you usually need to tune the knobs of your system to make sure the following things hold true
...
| ----------- | --------------------------------------------------------- |
| Algorithm A | 3% error (a lot of porn images are treated as cat images) |
| Algorithm B | 5% error                                                  |
...
when $$x^{(i)}$$ is porn $$w^{(i)} = 10$$, or $$x^{(i)}$$ is not porn $$w^{(i)} = 1$$
...
{% include image.html image="notes/machine-learning-strategy/01.png" %}
...
Traditional applications of supervised learning learned a function $$h : X \to Y$$, where the output $$y$$ was usually an integer or a real number. For example:
...
- Whereas in transfer learning, you have a sequential process where you learn from task A and then transfer that to task B. In multi-task learning, you start off simultaneously, trying to have one neural network do several things at the same time. And then each of these tasks helps hopefully all of the other tasks.
  ...
  - cost function : $$ \frac{1}{m} \sum_{j=1}^{4} \Big( \sum_{i=1}^{m} loss(\hat{y}_j^{(i)}, y_j^{(i)}) \Big) $$
  - loss function : $$ - y_j^{(i)} * log(\hat{y}_j^{(i)}) - (1 - y_j^{(i)}) * log(1 - \hat{y}_j^{(i)}) $$
...
- Multi-task learning will also work if y isn't complete for some labels. For example:
  ...
  - cost function : $$ \frac{1}{m} \sum_{j=1}^{4} \Big( \sum_{i=1}^{m} loss(\hat{y}_j^{(i)}, y_j^{(i)}) \Big) $$ where $$y_j^{(i)} \neq ?$$
